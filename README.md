...
1. `AirOutput.exe` 실행 → 작업 표시줄 트레이에 아이콘 생성
...
3. AirOutput [입력 소스] → **CABLE Input** (설치돼 있으면 자동 선택됨)
...
산출물: `bin\Release\net9.0-windows\win-x64\publish\AirOutput.exe` (.NET 9 Desktop Runtime 필요)
...
산출물: `installer\AirOutput.msi` (자립형 — 대상 PC에 .NET 불필요). 설치 내용:
...
| `Settings.cs` | `%APPDATA%\AirOutput\settings.json` |
